...
transformed parameters{
  ...
  // defining beta_prg = 0 here, so the variable needs to be declared in the same block
  ...
  real<lower=0> test_reas_f[ag,4,r];  // last year, women 4 separate pathways for testing (combining screening into one)
  real<lower=0> test_reas_mw[ag,3,r]; // last year, msw 3 separate pathways for testing
  real<lower=0> test_reas_mm[ag,3,r]; // last year, msm 3 separate pathways for testing
...
for (y in 9:timestep){
  ...
  for (y in 1:timestep){
    for (i in 1:ag){
      ...
      beta[i,1,2,y] = beta_scr[i,1,2] + tst_prg*beta_prg[i,y]; // hr women no HIV for beta
      beta[i,2,1,y] = beta_scr[i,2,1] ;                // lr MSW no HIV for beta
      beta[i,2,2,y] = beta_scr[i,2,2];                 // hr MSW no HIV for beta
      beta[i,3,1,y] = beta_scr[i,3,1] ;                // lr MSM no HIV for beta
      beta[i,3,2,y] = beta_scr[i,3,2] ;                // hr MSM no HIV for beta
      
      // hiv -
      beta2[i,1,1,1,y] = beta2_scr[i,1,1,1] + tst_prg*beta_prg[i,y];              // lr women 
      beta2[i,1,1,2,y] = beta2_scr[i,1,1,2] + tst_prg*beta_prg[i,y] + tst_prep*beta_prep[i,1,y] ; // hr women 
      beta2[i,1,2,1,y] = beta2_scr[i,1,2,1] ;                                    // lr MSW 
      beta2[i,1,2,2,y] = beta2_scr[i,1,2,2] + tst_prep*beta_prep[i,2,y] ;        // hr MSW 
      beta2[i,1,3,1,y] = beta2_scr[i,1,3,1] ;                                    // lr MSM 
      beta2[i,1,3,2,y] = beta2_scr[i,1,3,2] + tst_prep*beta_prep[i,3,y] ;        // hr MSM 

...
nsfg_recent_df <- data.frame( t = rep(rep(c(2012, 2014, 2016, 2018), each=3),2),
                       data_nsfg = c(as.vector(tested_recent_f2), as.vector(tested_recent_m2) ),  
                       age = rep(rep(c("15-24", "25-34", "35-44"),4),2) ,
                       population = rep( c("women", "msw"), each= length(as.vector(tested_recent_f2))))
...
smr_recpred <- cbind(as.data.frame( summary(
  ...
  age = age_string, t= rep(1999:2024, gen_groups)) # ,
 colnames(smr_recpred) <- make.names(colnames(smr_recpred)) # to remove % in the col names
 
 
 smr_recpred %>%
 filter(population != "msm") %>%
 left_join(nsfg_recent_df, by = c("t", "population", "age") )  %>%
     ggplot(smr_recpred, mapping = aes(x = t)) +
       geom_ribbon(aes(ymin = X2.5., ymax = X97.5., fill=population), alpha = 0.35) +
       geom_line(mapping = aes(x = t, y = X50., color=population)) + 
       geom_point(mapping = aes(y = data_nsfg/d_denom2), color="red", shape=21) +
...
pop_df <- data.frame( t = rep(rep(d_popsize_f[1,], each=7),3),
                       data = c(as.vector(d_popsize_f[2:8,]), 0.95*as.vector(d_popsize_m[2:8,]), 0.05*as.vector(d_popsize_m[2:8,])) ,  
                       age = rep(c("15-24", "25-34", "35-44", "45-54", "55-64", "65-74", "75+"), length(d_popsize_f[1,])*3) ,
                       pop = rep( c("Women", "MSW", "MSM"), each=length(as.vector(d_popsize_f[2:8,]))) )
...
smr_pop %>%
  left_join(pop_df, by = c("t", "pop", "age") )  %>%
    ggplot( mapping = aes(x = t)) +
...
smr_prev %>%
  left_join(hivknown, by = c("t", "population", "age") )  %>%
      ggplot(smr_prev, mapping = aes(x = t)) +
        ...
        facet_wrap(~population+age, ncol = 5, scales="free") + # , , 
       # ylim(c(0,0.01)) +
        labs(x = "Year", y = "Living with a diagnosed HIV") + theme_minimal() +
        theme(legend.position = "none",
              axis.text.x=element_text(angle=45, size = 8)) +
